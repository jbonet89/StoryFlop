...
export type MemberRole = "host" | "collaborator" | "participant";
export interface Room { id: string; code: string; name: string; owner_user_id: string; status: RoomStatus; deck: VoteValue[]; created_at: string; updated_at: string; last_activity_at: string }
export interface Member { id: string; room_id: string; user_id: string; display_name: string; avatar_key: string; role: MemberRole; default_participation_mode: ParticipationMode; joined_at: string; is_kicked: boolean }
