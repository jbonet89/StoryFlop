...
import { ParticipantManagementDialog } from "@/features/room/components/ParticipantManagementDialog";
...
export function PokerRoom({ snapshot, code, realtimeStatus }: { snapshot: RoomSnapshot; code: string; realtimeStatus: RealtimeStatus }) {
  ...
  const canFacilitate = canFacilitateRoom(snapshot.me.role);
  ...
  function startTaskRound(taskId: string) {
    ...
    void action(() => roomApi.startRound(taskId));
  }
  ...
  return <main className="room-app">
    ...
    <div className={`room-layout ${mobileBacklogOpen ? "backlog-open" : ""}`}><TaskList tasks={snapshot.tasks} activeTaskId={activeTask?.id} activeControls={hostControls} isHost={canFacilitate} onCreate={(draft: ValidatedTaskDraft) => action(() => roomApi.createTask(snapshot.room.id, draft.title, draft.description, draft.taskUrl))} onDelete={id => void deleteTask(id)} onClear={clearBacklog} onStart={taskId => { setMobileBacklogOpen(false); startTaskRound(taskId); }} onInspect={(task, mode) => { setMobileBacklogOpen(false); inspectTask(task.id, mode); }} onReorder={taskIds => action(() => roomApi.reorderTasks(snapshot.room.id, taskIds))} onClose={() => setMobileBacklogOpen(false)} />
      <section className="table-stage"><div className="round-heading"><span>{activeRound ? t("round", { number: activeRound.round_number }) : t("tableReady")}</span><h1>{activeTask?.title ?? t("whatEstimate")}</h1>{activeTask?.description && <p><LinkedText text={activeTask.description} /></p>}</div>
        ...
        <RoundHistory rounds={snapshot.rounds} tasks={snapshot.tasks} members={snapshot.memberDirectory} participations={snapshot.participations} votes={snapshot.votes} />
      </section>
    </div>
    ...
    {inspectedTask && <TaskDetailDrawer key={`${inspectedTask.id}-${taskDetailMode}`} task={inspectedTask} rounds={snapshot.rounds} participations={snapshot.participations} votes={snapshot.votes} estimateChanges={snapshot.estimateChanges} isHost={canFacilitate} initialMode={taskDetailMode} onClose={closeTaskDetail} onSave={draft => action(() => roomApi.updateTask(inspectedTask.id, draft.title, draft.description, draft.taskUrl))} onUpdateEstimate={estimate => action(() => roomApi.updateFinalEstimate(inspectedTask.id, estimate))} onDelete={() => void deleteTask(inspectedTask.id)} />}
    ...
    {participantsOpen && <ParticipantManagementDialog members={members} meId={snapshot.me.id} isHost={isHost} onlineIds={presence.onlineIds} presenceReady={presence.status === "en_directo" && presence.synced} onClose={() => setParticipantsOpen(false)} onRemove={memberIds => action(() => roomApi.removeRoomMembers(snapshot.room.id, memberIds))} onRoleChange={(memberId, role) => action(() => roomApi.setMemberRole(snapshot.room.id, memberId, role))} />}
