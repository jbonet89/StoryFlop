...
export function PlayerSeat({ member, position, compact, online, participationMode, roundActive, voted, vote, revealed, isMe, onReact }: { member: Member; position: SeatPosition; compact: boolean; online: boolean; participationMode: ParticipationMode; roundActive: boolean; voted: boolean; vote?: Vote; revealed: boolean; isMe: boolean; onReact: (emoji: string) => void }) {
  ...
  return <div className={`player-seat ${isMe ? "me" : ""} ${compact ? "compact" : ""} ${participationMode === "observer" ? "observer" : ""}`} style={style} data-player-id={member.id} data-participation-mode={participationMode}>
    <button className="seat-main" title={member.display_name} onClick={() => !isMe && setPickerOpen(open => !open)} aria-expanded={!isMe ? pickerOpen : undefined} aria-label={t("seatLabel", { name: member.display_name, role: member.role, mode: participationLabel, connection: connectionLabel, canReact: isMe ? "no" : "yes" })}>
